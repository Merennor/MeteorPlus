...
public class ConfigModifier {
	...
	private void changedProtection() {
		...
		if (positionProtection.get()) {
			if (spoofMode.get() == SpoofMode.Fake) {
				ss.set("camera", new ValueMap()
					.set("pos", new ValueMap()
						...
						.set("x", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().x + ConfigModifier.get().x_spoof.get()))
						.set("y", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().y))
						.set("z", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().z + ConfigModifier.get().z_spoof.get()))
					)
			...
			ss.set("camera", new ValueMap()
				.set("pos", new ValueMap()
					.set("_toString", () -> posString(false, true))
					.set("x", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().x))
					.set("y", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().y))
					.set("z", () -> Value.number(mc.gameRenderer.getMainCamera().getPosition().z))
				)
				.set("opposite_dim_pos", new ValueMap()
					.set("_toString", () -> posString(true, true))
					.set("x", () -> oppositeX(true))
	...
	private Value oppositeX(boolean camera) {
		double x = 0;
		if (positionProtection.get()) {
			x = camera ? mc.gameRenderer.getMainCamera().getPosition().x + ConfigModifier.get().x_spoof.get() : (mc.player != null ? mc.player.getX() + ConfigModifier.get().x_spoof.get() : 0);
		}
		else {
			x = camera ? mc.gameRenderer.getMainCamera().getPosition().x : (mc.player != null ? mc.player.getX() : 0);
		}
	...
	private Value oppositeZ(boolean camera) {
		double z = 0;
		if (positionProtection.get()) {
			z = camera ? mc.gameRenderer.getMainCamera().getPosition().z + ConfigModifier.get().z_spoof.get() : (mc.player != null ? mc.player.getZ() + ConfigModifier.get().z_spoof.get() : 0);
		}
		else {
			z = camera ? mc.gameRenderer.getMainCamera().getPosition().z : (mc.player != null ? mc.player.getZ() : 0);
		}
		Dimension dimension = PlayerUtils.getDimension();
	...
	private static Value yaw(boolean camera) {
		float yaw;
		if (camera) yaw = mc.gameRenderer.getMainCamera().getYRot();
	...
	private static Value pitch(boolean camera) {
		float pitch;
		if (camera) pitch = mc.gameRenderer.getMainCamera().getXRot();
	...
	private static Value biome() {
		...
		ResourceLocation id = mc.level.registryAccess().lookup(Registries.BIOME).get().getKey(mc.level.getBiome(BP).value());
	...
	private Value posString(boolean opposite, boolean camera) {
		Vec3 pos;
		if (camera) pos = mc.gameRenderer.getMainCamera().getPosition();
